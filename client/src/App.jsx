  ...
  return (
    <div style={st.container}>
      <header style={st.header}>
        <div style={st.logoArea}><div style={st.logoIcon}>DG</div><div style={st.logoDivider}></div><div><h1 style={st.title}>DELFINO GAVIÑA</h1><p style={st.subtitle2}>Inversiones</p><p style={st.subtitle}>Market Data</p></div></div>
      ...
      {activeTab === 'fx' && (<>
        ...
        <section style={{ marginTop: 48 }}><SH title="EVOLUCIÓN HISTÓRICA" /><p style={st.sectionSub}>Tipo de cambio implícito · AL30 CI</p><FxHistoryChart /></section>
      </>)}
...
const st = {
  ...
  logoArea: { display: 'flex', alignItems: 'center', gap: 0 },
  logoIcon: { fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 30, fontWeight: 700, color: 'var(--dg-cream, var(--neon))', letterSpacing: -2, lineHeight: 1, paddingRight: 12 },
  logoDivider: { width: 1, height: 40, background: 'var(--dg-steel, var(--border))', marginRight: 12, opacity: 0.5 },
  title: { fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 700, fontSize: 22, letterSpacing: 5, color: 'var(--dg-cream, var(--neon))', textShadow: 'var(--neon-glow)', lineHeight: 1.1 },
  subtitle2: { fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: 11, color: 'var(--text)', letterSpacing: 3, marginTop: 1, textTransform: 'uppercase' },
  subtitle: { fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: 10, color: 'var(--text-dim)', letterSpacing: 2, marginTop: 1 },
